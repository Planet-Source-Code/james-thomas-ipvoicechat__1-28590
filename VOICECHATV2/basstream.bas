...
Public psa(1) As DSBPOSITIONNOTIFY
...
Public Declare Sub RtlZeroMemory Lib "kernel32" (dest As Any, ByVal cbSize As Long)
...
Public Sub CopyBuffer(part As Integer)
    ...
    'send the data to the other PC
    frmChat.wskClient.SendData PrepareAVoice(Buffer)
    'Put #f, , Buffer
    'sz = sz + cur.lWrite
    'frmMain.lblSize = Str$(sz) ' send size info to the form label
    frmChat.stbStatus.Panels("Recording").Text = UBound(Buffer) & "@" & Timer
End Sub
